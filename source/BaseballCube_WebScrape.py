...
def scrapeYearlyStats(yearStart,yearEnd=2021):
    ...
    for year in range(yearStart,yearEnd+1):
        ...
        for team_name in schools:
            ...
            if not os.path.exists('../CapstoneBaseballData/Hitting/'+str(team_name).replace(" ","_")+'_'+str(year)+'_Hitting.csv'):
                # Create a file to write the parsed data to 
                hit_save_file = open('../CapstoneBaseballData/Hitting/'+str(team_name).replace(" ","_")+'_'+str(year)+'_Hitting.csv','w')
                ...
                # Create a file to write the parsed data to 
                pitch_save_file = open('../CapstoneBaseballData/Pitching/'+str(team_name).replace(" ","_")+'_'+str(year)+'_Pitching.csv','w')
                # write to that newly created file
